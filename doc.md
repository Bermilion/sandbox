...
│   ├── _preflight.scss        # Современный CSS reset (Tailwind-style)
│   ├── _rem.scss              # Базовый размер шрифта для rem
...
│   └── _grids.scss            # Конфигурация сеток для разных устройств
├── type/                       # Типографика
...
│   ├── css-variables/         # Универсальная система CSS-переменных
│   │   ├── _index.scss        # Экспорт модуля
│   │   ├── _registry.scss    # Реестр CSS-переменных
│   │   ├── _generator.scss   # Генератор CSS-переменных
│   │   └── _naming.scss      # Формирование имён переменных
...
│   ├── _test-colors.scss      # Тестирование цветовой системы
│   ├── _test-css-variables.scss # Тестирование CSS-переменных
│   └── _test-scale.scss       # Тестирование системы масштабов
├── certain/                    # Специфические стили
│   ├── _body.scss
│   ├── _link.scss
│   └── _index.scss
└── components/                  # Компоненты (устарело, перенесено в ui/)
    └── _link.scss
```
...
#### Core/_preflight.scss

**Зона ответственности:** Современный CSS reset в стиле Tailwind

**Функционал:**
- Полный сброс стилей браузера по умолчанию
- Устранение несоответствий между браузерами
- Оптимизация для современных CSS-возможностей
- Установка консистентной базы для стилизации

**Ключевые особенности:**
- `box-sizing: border-box` для всех элементов
- Сброс margin/padding/border
- Консистентная типографика
- Улучшенная стилизация форм
- Оптимизация для мобильных устройств
- Поддержка современных псевдоэлементов

#### Core/_rem.scss

**Зона ответственности:** Базовый размер шрифта для rem-единиц

**Функционал:**
- Устанавливает базовый размер шрифта для HTML
- Обеспечивает консистентное поведение rem-единиц
- Текущая настройка: `font-size: 50%` (1rem = 8px)

**Преимущества:**
- Упрощение математических расчетов
- Консистентность с 8px микромодульной сеткой
- Удобство использования с системой масштабов
...
#### UI/Base/_button.scss

**Зона ответственности:** Стили кнопок

**Функционал:**
- Базовые стили кнопок
- Модификаторы для различных состояний
- Адаптивность для разных устройств

#### UI/Base/_link.scss

**Зона ответственности:** Стили ссылок

**Функционал:**
- Базовые стили ссылок
- Модификаторы для различных состояний и контекстов
- Интеграция с цветовой системой

### Utils Components

#### UI/Utils/_icon.scss

**Зона ответственности:** Стили SVG-иконок
...
**Зона ответственности:** Конфигурация адаптивных сеток

**Функционал:**
- Определение параметров сетки для разных устройств
- Настройка ширины контейнера, отступов и колонок
- Адаптивная система на основе rem-единиц

**Структура конфигурации:**
```scss
$device-name: (
  'width': контейнерная ширина,
  'gap': отступы между колонками,
  'spacing': боковые отступы,
  'cols': количество колонок
);
```

**Доступные устройства:**
- `phone-portrait`: 41rem ширина, 4 колонки
- `phone-landscape`: 92rem ширина, 6 колонок
- `tablet-portrait`: 88rem ширина, 6 колонок
- `tablet-landscape`: 120rem ширина, 6 колонок
- `laptop`: 154.75rem ширина, 12 колонок
- `wide`: 160rem ширина, 12 колонок
...
Пакет использует универсальную систему CSS-переменных с реестром для отслеживания и генерации переменных.

### Универсальная система CSS-переменных

**Модуль:** `utils/css-variables/`

**Зона ответственности:** Централизованное управление CSS-переменными

**Компоненты:**
- `_registry.scss` - Реестр для отслеживания использованных переменных
- `_generator.scss` - Генерация CSS var() из имён
- `_naming.scss` - Формирование имён переменных с эффектами и alpha
- `_index.scss` - Экспорт модуля

**Основные функции реестра:**
```scss
// Регистрация переменной (избегает дублирования)
register-variable($registry-map, $name, $value)

// Проверка регистрации
is-registered($registry-map, $name)

// Получение значения
get-registered-value($registry-map, $name)
```

**Глобальные карты отслеживания:**
```scss
...
Генерация происходит в `:root` через миксин `generate-css-variables` с использованием новой системы реестра.
